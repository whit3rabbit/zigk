...
// - Checksum validation using IPv6 pseudo-header
// - Rate limiting for error responses (DoS prevention)
// - Hop limit validation for NDP messages (must be 255)
// - No replies to multicast source addresses
...
pub fn processPacket(iface: *Interface, pkt: *PacketBuffer) bool {
    ...
    // Validate ICMPv6 checksum using IPv6 pseudo-header
    ...
    return switch (icmpv6.msg_type) {
        ...
        // NDP messages - delegate to NDP module
...
/// Handle ICMPv6 Packet Too Big (PMTUD)
fn handlePacketTooBig(pkt: *PacketBuffer) bool {
    ...
    // IPv6 minimum MTU is 1280 (RFC 8200)
    if (new_mtu < 1280) {
...
/// Handle ICMPv6 Parameter Problem
...
fn handleNdpMessage(iface: *Interface, pkt: *PacketBuffer) bool {
    // Validate hop limit == 255 for NDP messages (RFC 4861 Section 6.1.1)
