...
pub fn processListenPacket(
    ...
    tcb.local_addr = IpAddr{ .v4 = ip_hdr.getDstIp() };
    tcb.local_port = tcp_hdr.getDstPort();
    tcb.remote_addr = IpAddr{ .v4 = ip_hdr.getSrcIp() };
    tcb.remote_port = tcp_hdr.getSrcPort();

    tcb.irs = tcp_hdr.getSeqNum();
    tcb.rcv_nxt = tcb.irs +% 1;
    tcb.iss = state.generateIsn(tcb.getLocalIpV4(), tcb.local_port, tcb.getRemoteIpV4(), tcb.remote_port);
...
/// Process packet for LISTEN state (IPv6)
pub fn processListenPacket6(
    iface: *state.Interface,
    listen_tcb: *Tcb,
    pkt: *PacketBuffer,
    tcp_hdr: *TcpHeader,
    ...
    // SECURITY: Acquire state.lock BEFORE checking half-open count
    var state_held = state.lock.acquire();
    ...
    const ip6_hdr = packet.getIpv6Header(pkt.data, pkt.ip_offset) orelse {
        state_held.release();
        return false;
    };
    tcb.local_addr = IpAddr{ .v6 = ip6_hdr.dst_addr };
    tcb.local_port = tcp_hdr.getDstPort();
    tcb.remote_addr = IpAddr{ .v6 = ip6_hdr.src_addr };
    tcb.remote_port = tcp_hdr.getSrcPort();

    tcb.irs = tcp_hdr.getSeqNum();
    tcb.rcv_nxt = tcb.irs +% 1;
    // Use polymorphic ISN generation for IPv6
    tcb.iss = state.generateIsnIp(tcb.local_addr, tcb.local_port, tcb.remote_addr, tcb.remote_port);
    tcb.snd_nxt = tcb.iss +% 1;
    tcb.snd_una = tcb.iss;
