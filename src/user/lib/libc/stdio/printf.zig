...
// For aarch64, export printf_impl which is called by the C shim.
// The C shim provides the variadic printf() entry point using __builtin_va_start.
pub export fn printf_impl(fmt_str: [*:0]const u8, ap_raw: ?*anyopaque) c_int {
    var ap = VaList.from(ap_raw);
    return printf_core(fmt_str, &ap);
}

// x86_64: Export the variadic printf directly
// On aarch64, the C shim provides printf and calls printf_impl
const X86PrintfExport = if (builtin.cpu.arch == .x86_64) struct {
    pub export fn printf(fmt_str: [*:0]const u8, ...) callconv(.c) c_int {
        var args = @cVaStart();
        defer @cVaEnd(&args);
