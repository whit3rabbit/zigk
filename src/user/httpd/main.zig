...
fn printError(err: syscall.SyscallError) void {
    const msg = switch (err) {
        ...
        error.NotASocket => "ENOTSOCK",
        error.OperationNotSupported => "EOPNOTSUPP",
        error.AddressFamilyNotSupported => "EAFNOSUPPORT",
        error.AddressInUse => "EADDRINUSE",
        error.AddressNotAvailable => "EADDRNOTAVAIL",
        error.NetworkDown => "ENETDOWN",
        error.NetworkUnreachable => "ENETUNREACH",
        error.ConnectionAborted => "ECONNABORTED",
        error.ConnectionReset => "ECONNRESET",
        error.NoBufferSpace => "ENOBUFS",
        error.AlreadyConnected => "EISCONN",
