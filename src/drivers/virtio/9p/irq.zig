// VirtIO-9P Interrupt Handling
//
// MSI-X setup and interrupt handler for VirtIO-9P device.
// VirtIO-9P uses a single queue, so we only need one vector.
//
// Reference: VirtIO Specification 1.2+ Section 4.1.4 (PCI MSI-X)

const std = @import("std");
const hal = @import("hal");
const console = @import("console");
const pci = @import("pci");

const root = @import("root.zig");
const config = @import("config.zig");

// ============================================================================
// MSI-X Setup
// ============================================================================

/// Configure MSI-X interrupts for the device
pub fn setupMsix(device: *root.Virtio9PDevice, pci_access: pci.PciAccess) !void {
    const ecam = switch (pci_access) {
        .ecam => |e| e,
        .legacy => {
            console.warn("VirtIO-9P: MSI-X not available with legacy PCI access", .{});
            return;
        },
    };

    // Find MSI-X capability
    const msix_cap = pci.capabilities.findMsix(&ecam, device.pci_dev) orelse {
        console.warn("VirtIO-9P: MSI-X capability not found, using polling", .{});
        return;
    };

    // Enable MSI-X
    const msix_alloc = pci.msi.enableMsix(&ecam, device.pci_dev, &msix_cap, 0) orelse {
        console.warn("VirtIO-9P: Failed to enable MSI-X", .{});
        return;
    };

    console.info("VirtIO-9P: MSI-X enabled with {} vectors", .{msix_alloc.vector_count});

    if (msix_alloc.vector_count == 0) {
        console.warn("VirtIO-9P: No MSI-X vectors available", .{});
        return;
    }

    // Allocate one vector for the request queue
    const vector = hal.interrupts.allocateMsixVector() orelse {
        console.warn("VirtIO-9P: No MSI-X vectors available in kernel pool", .{});
        return;
    };

    // Configure MSI-X table entry 0
    const configured = pci.msi.configureMsixEntry(
        msix_alloc.table_base,
        msix_alloc.vector_count,
        0, // Entry 0
        vector,
        0, // Target APIC ID 0 (BSP)
    );

    if (!configured) {
        _ = hal.interrupts.freeMsixVector(vector);
        console.warn("VirtIO-9P: Failed to configure MSI-X entry", .{});
        return;
    }

    // Register handler
    if (!hal.interrupts.registerMsixHandler(vector, p9IrqHandlerWrapper)) {
        _ = hal.interrupts.freeMsixVector(vector);
        console.warn("VirtIO-9P: Failed to register IRQ handler", .{});
        return;
    }

    device.msix_vector = vector;

    // Configure queue to use MSI-X vector 0
    device.common_cfg.queue_select = config.QueueIndex.REQUEST;
    hal.mmio.memoryBarrier();
    device.common_cfg.queue_msix_vector = 0;
    hal.mmio.memoryBarrier();

    if (device.p9queue) |*q| {
        q.msix_vector = 0;
    }

    console.info("VirtIO-9P: Request queue -> MSI-X vector {}", .{vector});

    // Enable all configured vectors
    pci.msi.enableMsixVectors(&ecam, device.pci_dev, &msix_cap);

    // Disable legacy INTx
    pci.msi.disableIntx(&ecam, device.pci_dev);
}

/// Cleanup MSI-X resources
pub fn cleanupMsix(device: *root.Virtio9PDevice) void {
    if (device.msix_vector) |vector| {
        hal.interrupts.unregisterMsixHandler(vector);
        _ = hal.interrupts.freeMsixVector(vector);
        device.msix_vector = null;
    }
}

// ============================================================================
// Interrupt Handler
// ============================================================================

/// Wrapper for MSI-X handler that matches HAL signature
fn p9IrqHandlerWrapper(_: *hal.idt.InterruptFrame) void {
    if (root.getDevice()) |device| {
        handleInterrupt(device);
    }
}

/// Handle interrupt from device
pub fn handleInterrupt(device: *root.Virtio9PDevice) void {
    // Read and clear ISR (if available)
    if (device.isr_addr != 0) {
        const isr_ptr: *volatile u8 = @ptrFromInt(device.isr_addr);
        _ = isr_ptr.*; // Reading clears the ISR
    }

    // Process completed requests
    if (device.p9queue) |*q| {
        q.processCompleted();
    }
}

// ============================================================================
// Legacy IRQ
// ============================================================================

/// Register legacy IRQ handler (fallback if MSI-X unavailable)
pub fn registerLegacyIrq(device: *root.Virtio9PDevice) void {
    const irq_line = device.pci_dev.irq_line;

    if (irq_line == 0 or irq_line == 255) {
        console.warn("VirtIO-9P: No IRQ line assigned, using polling", .{});
        return;
    }

    const vector = @as(u8, @intCast(irq_line)) + 32; // PIC offset

    hal.interrupts.registerHandler(vector, p9IrqHandlerWrapper);
    console.info("VirtIO-9P: Registered legacy IRQ {} (vector {})", .{ irq_line, vector });

    hal.apic.routeIrq(irq_line, vector, 0);
    hal.apic.enableIrq(irq_line);
}

// ============================================================================
// Polling Mode
// ============================================================================

/// Poll for completions (used when interrupts are unavailable or debugging)
pub fn pollCompletions(device: *root.Virtio9PDevice) u32 {
    if (device.p9queue) |*q| {
        var processed: u32 = 0;
        while (q.hasPending() and processed < 256) : (processed += 1) {
            q.processCompleted();
        }
        return processed;
    }
    return 0;
}
