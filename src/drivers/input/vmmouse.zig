...
//!
//! Uses the VMware "Backdoor" interface to retrieve absolute cursor coordinates.
...
pub const VmMouseDriver = struct {
    ...
    pub fn probe(self: *Self) bool {
        ...
        var regs = vmware.Registers{
            .eax = vmware.BACKDOOR_MAGIC,
            .ebx = VMMOUSE_CMD_READ_ID,
            .ecx = 0, // 0 for ID / Enable
            .edx = vmware.BACKDOOR_PORT,
            .esi = 0, 
            .edi = 0
        };
    ...
    pub fn poll(self: *Self) void {
        ...
        while (count >= 4) {
             ...
             for (0..4) |i| {
                 regs.eax = vmware.BACKDOOR_MAGIC;
                 regs.ebx = VMMOUSE_DATA;
                 regs.ecx = 0;
                 regs.edx = vmware.BACKDOOR_PORT;
                 vmware.call(&regs);
                 packet[i] = regs.eax;
             }
             ...
             // Valid coordinate check (some packets may be relative if not fully switched)
