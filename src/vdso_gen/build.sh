...
zig build-lib src/vdso_gen/vdso.zig -target x86_64-linux-none -dynamic -O ReleaseSmall -fstrip --name vdso -z max-page-size=16

# Check if libvdso.so exists
if [ -f "libvdso.so" ]; then
    mv libvdso.so src/vdso_gen/vdso.so
else
...
python3 -c "import sys; d=sys.stdin.buffer.read(); print(',\n'.join([', '.join(['0x{:02x}'.format(b) for b in d[i:i+16]]) for i in range(0, len(d), 16)]))" < src/vdso_gen/vdso.so >> src/kernel/vdso_blob.zig
echo "};" >> src/kernel/vdso_blob.zig

rm src/vdso_gen/vdso.so
